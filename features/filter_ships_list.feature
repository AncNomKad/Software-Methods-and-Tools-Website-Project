Feature: Display list of ships filtered by certain classes and manufacturer

  As an avid FreeSpacec fan
  So that I can filter out certain ship classes and manufacturers
  I want to be able to un-check some boxes to exclude certain classes or manufacturers from the database.
...
Scenario: filter ships by class

Scenario: filter ships by manufacturer